...
Actions → 「BTC 均线监控」→ **Run workflow** → 下拉框选 **`test-notify`** → 运行。

约 20 秒后微信会收到一条测试消息。这个测试**不会改动任何提醒状态**，随时可跑。
若 Secret 缺失或名字写错，运行会直接标红并在日志中说明原因。

> ⚠️ Server酱 免费版每天 5 条额度，测试也算在内，别反复点。
...
- **云端无状态**：每次运行完会把 state.json 提交回仓库，这样"哪条均线已经提醒
  过、还在冷却中"的记忆能跨次运行保留，不会重复轰炸。写回时不做合并——
  state.json 是"最后写入者胜出"的文件，做 rebase 只会在重跑旧运行或运行期间
  有人推送时冲突失败，正确做法是把新算出的内容直接叠加到远程最新提交之上。
